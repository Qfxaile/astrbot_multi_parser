...
- **管理员私聊登录**：当前可通过 B站二维码自动获取并保存插件公共 Cookie。
...
当前支持管理员在私聊中通过 B站客户端扫码配置插件级公共登录态。命令只接受中文平台名；非管理员不会进入登录流程，群聊中也不会发送二维码或展示登录状态。
...
`/平台登录 B站` 会发送一次性二维码并等待手机确认；成功后自动写入 `bilibili_cookies` 并保存插件配置。同一平台同一时间只允许一个登录流程，二维码过期或插件卸载时会清理临时会话。B站短信登录依赖额外的人机验证，当前纯私聊流程不会尝试绕过该验证，因此暂不提供验证码兜底。
