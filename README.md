...
- **管理员私聊登录**：当前可通过 B站或贴吧二维码自动获取并保存插件公共 Cookie。
...
当前支持管理员在私聊中通过 B站或百度客户端扫码配置插件级公共登录态。命令只接受中文平台名；非管理员不会进入登录流程，群聊中也不会发送二维码或展示登录状态。
...
`/平台登录 B站` 和 `/平台登录 贴吧` 会发送一次性二维码并等待手机确认；成功后分别写入 `bilibili_cookies` 或 `tieba_cookies` 并保存插件配置。同一平台同一时间只允许一个登录流程，二维码过期、取消登录或插件卸载时会清理临时会话。

贴吧使用百度账号官方 Web 二维码流程。该流程可能根据网络、设备或账号状态触发滑块、人机验证、设备验证或其他平台风控；插件会立即终止登录，不生成风控参数、不调用打码服务，也不使用短信登录绕过验证。登录成功后仅保存贴吧页面解析所需的百度登录 Cookie，并且只在请求 `tieba.baidu.com` 页面时发送。
